...
// === FALLBACK UNBLOCK: si PHP plante (fatale/timeout), débloque le spinner et logge l'erreur ===
register_shutdown_function(function() {
    ...
    if ($err && in_array($err['type'], $fatal_types)) {
        ...
        // Toujours émettre $.unblockUI() pour débloquer le spinner côté navigateur
...
if(isset($GLOBALS['PARAM']['GENERER_THEMES_PENDANT_SAISIE']) && count($GLOBALS['PARAM']['GENERER_THEMES_PENDANT_SAISIE'])>0){
	if(in_array($_GET['theme'],$GLOBALS['PARAM']['GENERER_THEMES_PENDANT_SAISIE'])){
		...
		$form	=	new frame( $id_themes, $langues, $id_systemes, $_SESSION['annee'], $_SESSION['code_etab'] );
	}
}					
...
<script type="text/Javascript" language="javascript">
	...
	function desactiver_zone(){
		...
		var chp_pere=args[0];
		liste_val_comp = args[1];
		var listZoneTab = args[2].split(",");
		if(chp_pere != 'undefined' && chp_pere != ''){
			tab_chp_pere = chp_pere.split('_');
			val_chp_pere = tab_chp_pere[tab_chp_pere.length-1] * 1;
			if(inArray(val_chp_pere,liste_val_comp) && $('#'+chp_pere).is(':checked')){
				bool = false;
			}else{
				bool = true;
			}
				...
				for (i=0; i<(tab_codes_chps.length); i++){
					...
					for(var j = 0; j<(tab_codes_chp.length); j++){
						...
						if(eval ('document.getElementById("'+nom_zone+'_0'+val_code+'")')){
							...
							//eval ('document.getElementById("'+nom_zone+'_0'+val_code+'").enabled = '+bool+';');
						}
					}
				}
			}
		...
		k = 0;
		liste_zones = "";
		for (i=0; i<(listZoneTab.length); i++){
			nom_zone = listZoneTab[i];
	...
	function run_suppression(annee, check_action){
		...
		xhr2.onreadystatechange = function(){
			...
			if(xhr2.readyState == 4 && xhr2.status == 200){
				...
				if(check_action == 'suppr_donnees') document.location.href='questionnaire.php?theme=<?php echo $_GET['theme']; ?>';
				else if(check_action == 'suppr_etab') document.location.href='saisie_donnees.php?val=choix_etablissement';
			}
		}

		//url="administration.php?val=imputation";
		url="saisie_donnees.php?val=suppr_etablissement&annee_suppr="+annee+"&action_suppr="+check_action;
		// Ici on va voir comment faire du get
	...
	function AlertSuppr(annee){
		if(annee != "" ){
			...
			?>
			if(document.getElementById('id_suppr_donnees').checked == true){check_action='suppr_donnees'}
			else if(document.getElementById('id_suppr_etab').checked == true){check_action='suppr_etab'}
			
			if(check_action != "" ){
				
				if(check_action == 'suppr_donnees'){var msg_alert = "<?php echo recherche_libelle_page('suppr_donnees');?>("+ tab_lib_ann[annee] +") ?";}
				else if(check_action == 'suppr_etab'){var msg_alert = "<?php echo recherche_libelle_page('suppr_etab');?> "+' ?';}
				
				if(confirm(msg_alert)){
					MM_displayLayers('suppr_running','','block');
					MM_showHideLayers_Suppr('form_suppr','','hide','suppr_running','','show');
					run_suppression(annee, check_action);
				}
	...
	function MM_showHideLayers_Suppr() { //v6.0
	  var i,p,v,obj,args=MM_showHideLayers_Suppr.arguments;
	  for (i=0; i<(args.length-2); i+=3) if ((obj=MM_findObj(args[i]))!=null) { v=args[i+2];
...
<script type="text/Javascript">	
	...
	// FALLBACK : débloque automatiquement le spinner si la page se charge normalement
	// (window.load se déclenche quand TOUT le HTML est reçu)
...
if(!isset($_GET['tmis'])){
	...
	<!-- Rappel des infos etablissement -->
		...
		<p <?php echo $style_infos_etab;?>><?php print $_SESSION['infos_etab']; ?></p>
		<?php }else{ ?>
		<a href="questionnaire.php?theme=<?php echo $_GET['theme']; if(isset($_SESSION['type_ent_stat'])) echo "&type_ent_stat=".$_SESSION['type_ent_stat'] ; if(isset($_GET['id_teacher'])) echo "&id_teacher=".$_GET['id_teacher'] ; if(isset($_GET['ligne'])) echo "&ligne=".$_GET['ligne'] ; ?>" style=" text-decoration:none ; " ><p <?php echo $style_infos_etab;?>><?php print '<b>' . $_SESSION['hierarchie_regroup'] . '</b>'; ?></p></a>
		<a href="questionnaire.php?theme=<?php echo $_GET['theme']; if(isset($_SESSION['type_ent_stat'])) echo "&type_ent_stat=".$_SESSION['type_ent_stat'] ; if(isset($_GET['id_teacher'])) echo "&id_teacher=".$_GET['id_teacher'] ; if(isset($_GET['ligne'])) echo "&ligne=".$_GET['ligne'] ; ?>" style=" text-decoration:none ; " ><p <?php echo $style_infos_etab;?>><?php print $_SESSION['infos_etab'].$_SESSION['infos_data_entry']; ?></p></a>
		<?php } ?>
		</div>
		<?php 
		if ((!$GLOBALS['PARAM']['HIDE_DELETE_SCHOOL'] or ( $GLOBALS['PARAM']['HIDE_DELETE_SCHOOL'] && ($_SESSION['groupe'] ==1))) && (!isset($_SESSION['tmis'])) && (!isset($_GET['ligne'])) ){
			
			if( isset($_SESSION['code_etab']) and (trim($_SESSION['infos_etab']) <> '')  ){ 
				$html_suppr = '<div id="show_form_suppr" style="position:absolute; top: 68px; right: 80px; z-index: 2;"><a href="#" onClick="MM_displayLayers(\'form_suppr\',\'\',\'block\'); MM_showHideLayers_Suppr(\'show_form_suppr\',\'\',\'hide\',\'hide_form_suppr\',\'\',\'show\',\'form_suppr\',\'\',\'show\',\'suppr_running\',\'\',\'hide\')"><strong>'.replaceAccents(recherche_libelle_page('SupEtab')).'>></strong></a></div>';
				$html_suppr .= '<div id="hide_form_suppr" style="position:absolute; top: 68px; right: 80px; z-index: 2;"><a href="#" onClick="MM_displayLayers(\'form_suppr\',\'\',\'none\'); MM_showHideLayers_Suppr(\'show_form_suppr\',\'\',\'show\',\'hide_form_suppr\',\'\',\'hide\',\'form_suppr\',\'\',\'hide\',\'suppr_running\',\'\',\'hide\')"><strong>'.replaceAccents(recherche_libelle_page('SupEtab')).'<<</strong></a></div>';
				
					$html_suppr .='<div id="form_suppr" style="top: 68px; right: 80px; z-index: 3;">
					<form  name="suppr_formulaire">
					  <table id="tabl_suppr">
						  <caption><strong> '.(recherche_libelle_page('supprtitl')).' </strong></caption>
						 <tr>
							  <td nowrap="nowrap">
								<input type="radio" name="action_suppr" value="suppr_donnees" id="id_suppr_donnees"> '.(recherche_libelle_page('actionsuppr')).'&nbsp;</td>
							  <td nowrap="nowrap">&nbsp;'.(recherche_libelle_page('yeardata')).'
								<select name="annee_suppr">';
									
									foreach ($list_ann as $i_ann => $ann){
										$html_suppr .= "<option value='".$ann[$GLOBALS['PARAM']['CODE'].'_'.$GLOBALS['PARAM']['TYPE_ANNEE']]."'";
										if($ann[$GLOBALS['PARAM']['CODE'].'_'.$GLOBALS['PARAM']['TYPE_ANNEE']] == $_SESSION['annee']){
											$html_suppr .= " selected ";
										}
										$html_suppr .= ">".$ann[$GLOBALS['PARAM']['LIBELLE'].'_'.$GLOBALS['PARAM']['TYPE_ANNEE']]."</option>";
									}
							
						$html_suppr .='</select></td>
						</tr>'; ?>
						<?php
						if ($_SESSION['groupe']==1){
							$html_suppr .=
							'<tr>
							  <td colspan="2" nowrap="nowrap">
								<input type="radio" name="action_suppr" value="suppr_etab"  id="id_suppr_etab">
								'.(recherche_libelle_page('supp_retab')).' </td>
							</tr>';
						}
							$html_suppr .=
							'<tr>
							  <td colspan="2" nowrap="nowrap">
								  <div align="right">
									<input name="ok" type="button" onClick="MM_callJS(\'AlertSuppr(document.suppr_formulaire.annee_suppr.value)\')" value="'.(recherche_libelle_page('submit')).'">
									&nbsp;&nbsp;
									  <input name="annuler" type="button" onClick="MM_displayLayers(\'form_suppr\',\'\',\'none\'); MM_showHideLayers_Suppr(\'show_form_suppr\',\'\',\'show\',\'hide_form_suppr\',\'\',\'hide\',\'form_suppr\',\'\',\'hide\')" value="'.(recherche_libelle_page('closeest')).'">
								  </div>
						 </td></table>
					  </form>
				  </div>
				  
					<div id="suppr_running" style="top: 68px; right: 80px; z-index: 3;">
						
						<table width="100%">
						  <caption><strong> '.recherche_libelle_page('supprtitl').' </strong></caption>
						 <tr><td height="28" style="margin:0px">
							<div id="suppr_progress"  align="center">
...
if (empty($theme_manager->id)) {
    ...
    echo '<script type="text/Javascript">$.unblockUI(); alert("Thème introuvable pour ce type établissement. Veuillez relancer depuis la liste des établissements.");</script>';
...
<?php if( $GLOBALS['pb_new_etab'] == true ){
	...
	print '</script>';
		...
		<?php $theme_precedant					= $theme_manager->recherche_theme_prec();
				...
				if(!isset($_SESSION['liste_etab'])){
					if($theme_precedant) {
						if(!$GLOBALS['PARAM']['HIDE_THEME_NAVIG'] || $_SESSION['groupe']==1){
	...
	if( isset($_SESSION['imput_all_tabms'][$_SESSION['secteur']]) && count($_SESSION['imput_all_tabms'][$_SESSION['secteur']])  ) {
				...
				if( ($GLOBALS['conn']->Execute($reqtmpdata)) === false ){}else{
					...
					if( is_array($existtmpdata) && count($existtmpdata) > 0){
						...
						if($exist_chp_imput){
							...
							?>
								<div id="info_imput"><span id="info_imput_span" class="StyleImputData" style="width:135px; height:auto"><?php echo recherche_libelle_page('tamponimp');?></span></div>
							<?php
							}
							$curr_thm_imput = true ;
						}
		...
		if( $_SESSION['groupe'] ==1 || $_SESSION['groupe'] ==2  || $_SESSION['groupe'] ==3){
			?>
			<?php
...
?>
<?php 
if((isset($_GET['ctrl']) && $_GET['ctrl'] == 1 && isset($_SESSION['num_lig_page']) && isset($_SESSION['nbre_lig_page']))
	||(isset($_GET['ctrl']) && $_GET['ctrl'] == 1 && isset($_GET['num_lig_page']) && isset($_GET['nbre_lig_page']))){
	if((isset($_GET['ctrl']) && $_GET['ctrl'] == 1 && isset($_SESSION['num_lig_page']) && isset($_SESSION['nbre_lig_page'])))
		echo "MiseEvidenceLigneFrame('".$_SESSION['num_lig_page']."','".$_SESSION['nbre_lig_page']."');\n";
	elseif((isset($_GET['ctrl']) && $_GET['ctrl'] == 1 && isset($_GET['num_lig_page']) && isset($_GET['nbre_lig_page'])))
		echo "MiseEvidenceLigneFrame('".$_GET['num_lig_page']."','".$_GET['nbre_lig_page']."');\n";
}
?>
</script>
