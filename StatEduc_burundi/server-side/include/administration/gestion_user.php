...
if (isset($_POST["import"])) {
    ...
    if (in_array($_FILES["file"]["type"], $allowedFileType)) {
		...
		if (isset($_SESSION['instance_nomenc'] )){
			...
			// RÈcupÈration de la valeur du post
   ...
   unset($_SESSION['instance_nomenc']);   
	...
	$html .= '<div>';
	$html .= ' <label>'.$user->recherche_libelle_page('ChooseExcelFile',$_SESSION['langue'],'user').'</label> <input type="file" name="file" id="file" accept=".xls,.xlsx">';
	...
	$html .= '</div>';
	$html .= '</form>';	   
	// Bouton t√©l√©chargement du canevas Excel (session 68)
	$html .= '<div style="margin-top:8px;"><a class="btn btn-default" href="'.$GLOBALS["SISED_URL"].'administration.php?val=download_user_template" style="text-decoration:none;">&#11015; T√©l√©charger le canevas Excel (11 colonnes)</a></div>';
	$html .= '</div>';
	...
	if(!empty($message)) { $html .= $message; } 
		...
		$html .= '</caption>';
		$html .= '<thead><tr class="ligne-titre"><th>Num</th><th>Nom utilisateur</th><th>Email</th><th>Tel</th><th>Login</th><th>Groupe</th><th>Code √©tab.</th><th>Camp.</th><th>Message</th></tr></thead>';
	...
	$html .= $user->entete_template; 
   	...
   	// recherche des libellÈs des entÍtes du template
