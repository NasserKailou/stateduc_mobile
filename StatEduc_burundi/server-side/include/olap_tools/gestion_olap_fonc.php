...
<?php //$GLOBALS['PARAM']['']
		$champs = array();
		$champs[] = array('nom'=>'FONCTION', 'type'=>'text', 'cle'=>'1', 'incr'=>'', 'val'=>'', 'lib'=>'fonction', 'obli'=>'1', 'filtre'=>'', 'ordre'=>'');
		
		$table 						= new gestion_table_simple() ;
		$table->conn				= $GLOBALS['conn_dico'];
		
		$table->table				= 'DICO_OLAP_MES_FONCTION';
		$table->champs 				= $champs;
		$table->nom_champ_combo		= 'FONCTION';
		$table->frame				= $GLOBALS['SISED_PATH_INC'] . 'olap_tools/frame_gestion_olap_fonc.php';
		$table->taille_ecran		= '600';
		$table->taille_combo		= '100';
		$table->btn_quit			= false;
		$table->titre_ecran			= 'gest_fonc';
