    ...
    public function create_entete($init_depht=0, $final_depht=false, $combo_br=false) {
        ...
        for($i=$init_depht; $i<=$final_depht; $i++) {
            ...
            if($i == $init_depht) {
                ...
                if(!isset($_GET['code_regroup_' . $i]) && (!isset($_SESSION['rattachements'][$this->code_chaine][$i]) || $_SESSION['rattachements'][$this->code_chaine][$i]=='')) {

                    $code_regroups[$i] = $regroupement[0][$GLOBALS['PARAM']['CODE'].'_'.$GLOBALS['PARAM']['REGROUPEMENT']];

                } elseif(isset($_GET['code_regroup_' . $i])) {
					unset($_SESSION['rattachements'][$this->code_chaine]);
                    $code_regroups[$i] = $_GET['code_regroup_' . $i];

                }elseif($_SESSION['rattachements'][$this->code_chaine][$i]<>''){
					$code_regroups[$i] = $_SESSION['rattachements'][$this->code_chaine][$i];
				
				}

                $j = 0;
				$combo_regroups[$i] = '<tr><td width="40%">'.$this->get_libelle_type_reg($curcodetyperegroup).'</td>'."\n".'<td  width="60%">';
                $combo_regroups[$i] .= '<select name="combo_regroup_'.$i.'" onchange="toggle_combo_'.$i.'();" style=\'width:100%;\'>' . "\n";
                
				$tmp_regroupement = array();
				
				if($final_depht == $i){
					if($_GET['option_reg'] == 'IN_REG'){
						foreach($regroupement as $i_regrp => $regrp) {
							if( in_array($regrp[$GLOBALS['PARAM']['CODE'].'_'.$GLOBALS['PARAM']['REGROUPEMENT']],$GLOBALS['regs_etab']) ){
								$tmp_regroupement[] = $regrp;
							}
						}
						$regroupement = $tmp_regroupement ;
					}elseif($_GET['option_reg'] == 'NOT_IN_REG'){
						foreach($regroupement as $i_regrp => $regrp) {
							if( !in_array($regrp[$GLOBALS['PARAM']['CODE'].'_'.$GLOBALS['PARAM']['REGROUPEMENT']],$GLOBALS['regs_etab']) ){
								$tmp_regroupement[] = $regrp;
							}
               ...
               foreach($regroupement as $i_regrp => $regrp) {	
                    ...
                    if($regrp[$GLOBALS['PARAM']['CODE'].'_'.$GLOBALS['PARAM']['REGROUPEMENT']] == $code_regroups[$i]) {
        ...
        foreach($code_regroups as $i=>$code_regroup) {
            ...
            for($j=0; $j<=$i; $j++) {
				...
				$temp_code .= "\t".'if (document.combo_regroups)  ' . "\n";
				$temp_code .= "\t".'var combo_'.$j.' = document.combo_regroups.combo_regroup_'.$j . ';' . "\n";
				$temp_code .= "\t".'else if (document.Formulaire) ' . "\n";
                $temp_code .= "\t".'var combo_'.$j.' = document.Formulaire.combo_regroup_'.$j . ';' . "\n";
               
				$temp_code .= "\t".'combo_'.$j.'_val = combo_'.$j.'.options[combo_'.$j.'.selectedIndex].value;' . "\n";
            }
			...
			$temp_code 		.= "\t".'document.location.href = \''.$curpage.'?' . $gets . $vars ;
			
			if($final_depht == $i){
				$GLOBALS['href_chg_etab'] = $temp_code ;
			}
