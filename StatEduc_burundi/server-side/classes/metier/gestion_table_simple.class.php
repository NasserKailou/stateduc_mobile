...
class gestion_table_simple{
	...
	public $btn_gen_params = false;
	public $act_Regen_params = false;
		...
		*/   
		...
		public function __construct(){
				...
				$this->btn_gen_params       =   false;
				$this->act_Regen_params     = false;
		}
		...
		*/
		...
		$this->text_alert['AvSup'] 			= $this->recherche_libelle(200,$this->langue,'DICO_MESSAGE');
						   ...
						   $this->text_alert['AvRegen'] = recherche_libelle_page('AvRegen');
   						   $this->text_alert['OkRegen'] = recherche_libelle_page('OkRegen');
				...
				<script type="text/javascript">
					<!--
						...
						function mess_MAJ(action,res_action,lib_champ_err){ 
							...
							//ajout
							else if(action=='Regen_param' ) i = 3; 
							...
							//ajout 
 							OK30 = " <?php echo $this->text_alert['PbRegen']; ?> ";
					        OK31 = " <?php echo $this->text_alert['OkRegen']; ?> ";
							
							var OK  = Array([OK00,OK01],[OK10,OK11],[OK20,OK21],[OK30,OK31]); 
					        if(action=='Add' || action=='Upd' || action=='Del') alert(OK[i][res_action] + lib_champ_err);
						...
						function popUpConfirm(i_enr,msg,action) {
						         $( "#confirm_content" ).empty();
						         $( "#confirm_content" ).append(msg);
						         $( "#div_confirm" ).dialog({
						         dialogClass: "toTop",   
						         resizable: false,
						         height:155,
						         modal: true,
						         buttons: {
						         	"<?php echo $this->recherche_libelle(101,$this->langue,'DICO_MESSAGE');?>": function() {
						                                        $( this ).dialog( "close" );
						                                        load_action(i_enr,action);
						                             },
						                   "<?php echo $this->recherche_libelle(209,$this->langue,'DICO_MESSAGE');?>": function() {
						                                        $( this ).dialog( "close" );
						                             }
						          }
						          }); 
				...
				if($this->total_enr == 0){ // pas d'enregistrement
						...
						if( (!isset($this->i_enr)) or (trim($this->i_enr) == '') ){
								...
								if($this->btn_add <> true){ // s'i ne s'agit pas d'un ajout
										...
										//ajout
										if(!in_array('btn_gen_params',$this->hidden_btn)) $this->btn_gen_params= true;
								}
								if( $this->i_enr > 0 ){
				...
				if($this->ok_action == 1){ // s'il n'y a pas deja une erreur dans les données postées
						...
						foreach($tab_req as $i => $tab){
								...
								elseif($tab['type']=='text'){
										$sql_values .=  $this->conn->qstr((convertWordSpecialChr(trim($tab['val'])))) ;
								}
						}
						$sql_champs .= ') ';
						$sql_values .= ') ';
						...
						if ( ($this->ok_action == 1) and (count($this->tab_champs_trad) > 0)  ){
								foreach($this->tab_champs_trad as $champ => $champs_trad){
										$requete 	= 'INSERT INTO DICO_TRADUCTION 
																 ...
																 VALUES
																 ( '.$_POST[trim($champ)].', \''. trim($champs_trad['table']) .'\', \''.$this->langue.'\', '.$this->conn->qstr((convertWordSpecialChr($_POST[$champs_trad['libelle']]))).' )';
										...
										insert_traduction('DICO_TRADUCTION', $_POST[trim($champ)], trim($champs_trad['table']), $this->langue, (convertWordSpecialChr($_POST[$champs_trad['libelle']])), 1);
								}
						}
				}
				if($this->ok_action == 0){
				...
				if($this->ok_action == 1){ // s'il n'y a pas deja une erreur dans les données postées
						...
						foreach($this->champs as $i => $champ){								
                        	if( !(isset($champ['cle']) and ($champ['cle'] == '1')) ){   
								...
								if(trim($_POST[$champ['nom']]) == ''){
										...
										elseif($champ['type']=='text'){
												$sql .= $champ['nom'] . '=' . $this->conn->qstr((convertWordSpecialChr(trim($_POST[$champ['nom']])))) ;
										}
								}
							}
						}
						foreach($this->champs as $i => $champ){
								if( isset($champ['cle']) and ($champ['cle'] == '1') ){
										...
										elseif($champ['type']=='text'){
												$sql .= $champ['nom'] . '=' . $this->conn->qstr((convertWordSpecialChr(trim($donnees_enr[$this->get_champ_extract($champ['nom'])]))));
										}
								}
						}
						$requete = $sql ;
						...
						if ( ($this->ok_action == 1) and (count($this->tab_champs_trad) > 0)  ){
								foreach($this->tab_champs_trad as $champ => $champs_trad){
										$requete 	= 'UPDATE DICO_TRADUCTION 
																 SET LIBELLE = '.$this->conn->qstr((convertWordSpecialChr($_POST[trim($champs_trad['libelle'])]))).'
		...
		*/
		...
		<FORM name="Formulaire"  method="post" action="<?php echo $PHP_SELF.'?'.$_SERVER['QUERY_STRING']; ?>">
			...
			<?php if( $this->btn_gen_params==true && $_GET['val']=='params_global' ){ ?>
				<input type='button' name='' <?php echo 'value="'.recherche_libelle_page('Regen').'"'; ?><?php echo "onClick=\"javascript:popUpConfirm('".($this->total_enr - 1)."','".recherche_libelle_page('AvRegen')."','Regen_param');\""; ?>>
												<?php }?>
...
public function alerte_MAJ(){
	//ajout
	if( (isset($this->act_MAJ) && (!$this->is_list || (trim($this->lib_champ_err)!=''))) || isset($this->act_Regen_params)){ // si on est aprés soumission
		...
		*/
		...
		public function putQuote($valeur){
            $table = ['true','false','array(','$GLOBA'];
            return in_array(substr($valeur,0,6),$table)? true : false;
	}
