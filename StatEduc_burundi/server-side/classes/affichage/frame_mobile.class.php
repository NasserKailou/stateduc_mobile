 ...
 * Genere les fichiers ws_mob_*.frame utilises par le WebView Flutter.
 *
 * MODIFIE session 23 (2026-06-23) :
 *   Ajout de _mobile_libelle_clean() pour corriger :
 *     Bug A : entites HTML brutes (&lt;b&gt;) dans le WebView mobile
 *     Bug B : mojibake ISO-8859-1 -> UTF-8 (accents corrompus)
 *   Les libelles de zones passent desormais par mb_convert_encoding + strip_tags
 *   avant ecriture dans le fichier frame (UTF-8 propre, sans balises HTML).
 *
...
//----------------------------------------------------------------------
	...
	function js_Post_Form($id_theme, $id_systeme){
		...
		foreach($ctrlList as $i_elem => $element){
			$ctrlJsElt .= '				ctrlSaisieLst['.$i_elem.'] = new Array('.$element['ID_ZONE'].', "'.$element['CHAMP_PERE'].'", "'.$element['TYPE_OBJET'].'", "'.$operateurs[$element['CTRL_SAISIE_OPERATEUR']].'", "'.$element['CTRL_SAISIE_EXPRESSION'].'", "'.htmlEncode($element['CTRL_SAISIE_MESSAGE']).'");'."\n";
		}
    ...
    function generer_frame_matrice_2D($id_theme, $langue ,$id_systeme){	
		...
		if($affiche_totaux_mat_2Dim){
			if ($this->dico[0][MESURE2]){
				...
				$html		.="\t\t<td align='center' rowspan='2' bgcolor='#CCCCCC'  valign='middle'><br>"."<INPUT TYPE= 'text' readonly='1' style='background-color:#CCCCCC; text-align: right; font-weight:bold' NAME='TOT_".$this->dico[0][MESURE1]."_COLONNE' ".$this->dico[0]['ATTRIB_OBJET']."></TD>\n";
				$html	.= "<script language='javascript' type='text/javascript'>"."\n";
				$html	.= 'tot_tab_col[0]	= "TOT_'.$this->dico[0][MESURE1].'_COLONNE";'."\n";
				$html	.= "</script>"."\n";
				$html		.="\t\t<td align='center' rowspan='2' bgcolor='#CCCCCC'  valign='middle'><br>"."<INPUT TYPE= 'text' readonly='1' style='background-color:#CCCCCC; text-align: right; font-weight:bold' NAME='TOT_".$this->dico[0][MESURE2]."_COLONNE' ".$this->dico[0]['ATTRIB_OBJET']."></TD>\n";
				$html	.= "<script language='javascript' type='text/javascript'>"."\n";
				$html	.= 'tot_tab_col[1]	= "TOT_'.$this->dico[0][MESURE2].'_COLONNE";'."\n";
				$html	.= "</script>"."\n";
				$html		.="\t\t<td align='center' rowspan='2' bgcolor='#CCCCCC'  valign='middle'><br>"."<INPUT TYPE= 'text' readonly='1' style='background-color:#CCCCCC; text-align: right; font-weight:bold' NAME='TOT_ALL_MES' ".$this->dico[0]['ATTRIB_OBJET']."></TD>\n";
    ...
    function generer_frame_matrice($id_theme, $langue ,$id_systeme){
        ...
        switch (count($this->dico)){
            ...
            case 2 :
                ...
                break;
            default :
                print 'Attention verifier le Dico (matrice)';
                break;
        }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		function formulaire_concat_zone_html($element,$langue,$id_systeme){
				...
				elseif($element['TYPE_OBJET']=='text'){
						...
						$html.= "".' ID=\''. $element['CHAMP_PERE']."_0" .'\' '."VALUE=\"\" $TOTAL_ChpsFrml>\n";
				...
				elseif($element['TYPE_OBJET']=='combo'){
						...
						$html		.= "\t\t<SELECT ".$element['ATTRIB_OBJET']."".' ID=\''. $element['CHAMP_PERE']."_0" .'\' '." NAME=".$element['CHAMP_PERE']."_0"." data-mini='true'>\n";
		...
		function get_cell_matrice($ligne ,$code_dims, $element, $fonc_total, $langue, $id_systeme){
				...
				elseif($element['TYPE_OBJET']=='text'){
						//////////////////////
	$html 	        .= "<div class='input_text'></div><input name_base='".$element['CHAMP_PERE']."_#_$code_dims' id='".$element['CHAMP_PERE']."_".$ligne."_".$code_dims."' name='".$element['CHAMP_PERE']."_".$ligne."_".$code_dims."' type='text' value=''>";
				...
				elseif($element['TYPE_OBJET']=='combo'){
						...
						$html	.= "\t\t<SELECT ".' ID=\''. $element['CHAMP_PERE']."_".$ligne."_".$code_dims .'\' '."".$element['ATTRIB_OBJET']." NAME=".$element['CHAMP_PERE']."_".$ligne."_".$code_dims." tabindex='".$GLOBALS['cell_mat_index']."' data-mini='true'>\n";
						$html 	.= "\t\t\t<OPTION VALUE=''>---</OPTION>\n";
						$result_nomenc 	= $this->requete_nomenclature($element['TABLE_FILLE'], $element['CHAMP_FILS'], $element['CHAMP_PERE'], $langue ,$id_systeme, $element['SQL_REQ']); 
						foreach($result_nomenc as $element_result_nomenc){
								//Un valeur du combo pour chaque valeur de la nomenclature 
...
// @auteur kailounasser@gmail.com - Abdoul Nasser Kailou — fix VALUE get_cell_matrice liste_radio
				...
				//Ajout HEBIE pour gestion champs cach�s
						...
						$html.= "<INPUT ".' name_base=\''.$element['CHAMP_PERE']."_#_".$code_dims.'\' ID=\''. $element['CHAMP_PERE']."_".$ligne."_".$code_dims .'\' '."NAME='".$element['CHAMP_PERE']."_".$ligne."_".$code_dims."' TYPE='hidden' ";
					...
					foreach($dico as $element){
						if( ( $element['TYPE_OBJET'] <> 'dimension_ligne')  && ( $element['TYPE_OBJET'] <> 'dimension_colonne') ){ // si type dimension on ignore
									...
									if( count($mat_codes_ligne) || count($mat_codes_colonne) ){
										...
										if(count($mat_codes_ligne)){
											...
											foreach($mat_codes_ligne as $li => $elem_li){
												...
												$html 		        .= "\n\t<TR>\n";
												// On imprime l'ent�te de ligne
												$html 		.= "\n\t\t<TD CLASS='".$classe_fond."' ".$mat_dim_ligne['ATTRIB_OBJET'].">$nbsp".$elem_li['libelle']."</TD>\n";
												if(count($mat_codes_colonne)){
													$tmp_i_mes=1;
													$cpt_i_mes=1;
													foreach($mat_codes_colonne as $col => $elem_col){
														foreach($mat_liste_mes as $i_mes => $nom_champ_mes){					
														   // Pour chaque colonne, on imprime la (ou les) zone(s) de texte
															...
															$html		.= "\t\t<TD nowrap='nowrap' align='center' valign='middle' CLASS='".$classe_fond."'>";
										...
										if(count($affiche_totaux_mat_Frml)){
											if (count($affiche_totaux_mat_Frml) > 1) {
												if(count($mat_codes_ligne)){
													...
													if(count($mat_codes_colonne)){
														foreach($affiche_totaux_mat_Frml as $i_mes => $tot_mes){
															$html		.="\t\t<td align='center' $rowspan_chp_tot bgcolor='#CCCCCC'  valign='middle'>$br_chp_tot"."<INPUT TYPE= 'text' readonly='1' style='background-color:#CCCCCC; text-align: right; font-weight:bold;'  " .$attrib_obj_mesures[$i_mes]. " NAME='TOT_".$element['ID_ZONE']."_".$mat_liste_mes[$i_mes]."_COLONNE_0'></TD>\n";
														}
													}
													$html		.="\t\t<td align='center' $rowspan_chp_tot bgcolor='#CCCCCC'  valign='middle'>$br_chp_tot"."<INPUT TYPE= 'text' readonly='1' style='background-color:#CCCCCC; text-align: right; font-weight:bold;' NAME='TOT_".$element['ID_ZONE']."_ALL_MES_0' " .$attrib_obj_mesures[$i_mes]. "></TD>\n";
													$html 		    .="\n\t</TR>\n";
													$html 		    .= "\n\t<TR>\n";
											...
											if(count($mat_codes_ligne)){
												...
												foreach($mat_codes_ligne as $li => $elem_li){
													...
													foreach($mat_liste_mes as $i_mes => $nom_champ_mes){
														...
														$html		.="\t\t<TD  nowrap='nowrap' align='center' valign='middle' CLASS='".$classe_fond."'>".$tab_libelles_mesures[$i_mes]."</TD>\n";
														...
														if(count($affiche_totaux_mat_Frml)){
															...
															if(count($mat_codes_colonne)){
																...
																if(count($affiche_totaux_mat_Frml) == 1){
																	$html		.="\t\t<TD  align='center' bgcolor='#CCCCCC'>"."<INPUT TYPE= 'text' readonly='1' style='background-color:#CCCCCC; text-align: right; font-weight:bold;' NAME='TOT_".$element['ID_ZONE']."_ALL_MES_LIGNE_0_".$elem_li['code']."' ".$element['ATTRIB_OBJET']."></TD>\n";
																}else{
																	//foreach($affiche_totaux_mat_Frml as $i_mes => $tot_mes){
																		$html	.="\t\t<TD  align='center' bgcolor='#CCCCCC'>";
																		if(!in_array($mat_liste_mes[$i_mes],$tab_champ_cache))
																			$html	.="<INPUT TYPE= 'text' readonly='1' style='background-color:#CCCCCC; text-align: right; font-weight:bold;' NAME='TOT_".$element['ID_ZONE']."_".$mat_liste_mes[$i_mes]."_LIGNE_0_".$elem_li['code']."' ".$element['ATTRIB_OBJET'].">";
																		$html	.="</TD>\n";
																		$html	.= "<script language='javascript' type='text/javascript'>"."\n";
															...
															if (count($affiche_totaux_mat_Frml) > 1) {
																...
																if(($k==1) && (!$affiche_sous_totaux))
																	$html		.="\t\t<TD $rowspan2 align='center' bgcolor='#CCCCCC' style='vertical-align:middle'>"."<INPUT TYPE= 'text' readonly='1' style='background-color:#CCCCCC; text-align: right; font-weight:bold;' NAME='TOT_".$element['ID_ZONE']."_ALL_MES_LIGNE_0_".$elem_li['code']."' ".$element['ATTRIB_OBJET']."></TD>\n";
																$k++;
															}	
														}
														$html 		    .= "\n\t</TR>\n";
													...
													if($affiche_sous_totaux){
														$html 			.= "\n\t<TR>\n";
															// On imprime le libell� des mesures
															if($element['LIB_EXPR']<>'')
																$html		.="\t\t<TD  align='center' valign='middle' CLASS='".$classe_fond."'>".trim($element['LIB_EXPR'])."</TD>\n";	
															else
																$html		.="\t\t<TD  align='center' valign='middle' CLASS='".$classe_fond."'>".$this->recherche_libelle_page('sous_total', $langue, 'questionnaire.php')."</TD>\n";
															if(count($mat_codes_colonne)){
																foreach($mat_codes_colonne as $col => $elem_col){
																	$html		.="\t\t<TD align='center' bgcolor='#CCCCCC'>"."<INPUT TYPE= 'text' readonly='1' style='background-color:#CCCCCC; text-align: right; font-weight:bold' NAME='TOT_".$element['ID_ZONE']."_ALL_MES_COLONNE_".$elem_li['code']."_".$elem_col['code']."' ".$element['ATTRIB_OBJET']."></TD>\n";
																}
																$html		.="\t\t<TD align='center' bgcolor='#CCCCCC' style='vertical-align:middle'>"."<INPUT TYPE= 'text' readonly='1' style='background-color:#CCCCCC; text-align: right; font-weight:bold;' NAME='TOT_".$element['ID_ZONE']."_ALL_MES_LIGNE_0_".$elem_li['code']."' ".$element['ATTRIB_OBJET']."></TD>\n";
															}
														$html 		    .= "\n\t</TR>\n";
													}
													...
													foreach($mat_liste_mes as $i_mes => $nom_champ_mes){
														...
														$html 			.= "\n\t<TR>\n";	
														// On imprime le libell� des mesures
														$html		.="\t\t<TD $colspan nowrap='nowrap' align='center' valign='middle' CLASS='".$classe_fond."'>".$tab_libelles_mesures[$i_mes]."</TD>\n";
														...
														if(count($affiche_totaux_mat_Frml)){
															...
															if(count($mat_codes_colonne)){
																...
																//foreach($affiche_totaux_mat_Frml as $i_mes => $tot_mes){
																	$html		.="\t\t<TD $rowspan_chp_tot align='center' bgcolor='#CCCCCC'>"." $br_chp_tot <INPUT TYPE= 'text' readonly='1' style='background-color:#CCCCCC; text-align: right; font-weight:bold;' NAME='TOT_".$element['ID_ZONE']."_".$mat_liste_mes[$i_mes]."_LIGNE_0' ".$element['ATTRIB_OBJET']."></TD>\n";
																//}
															}
															if (count($affiche_totaux_mat_Frml) > 1) {
																//$html		.="\t\t<TD $rowspan_chp_tot align='center' bgcolor='#CCCCCC'>"." $br_chp_tot <INPUT TYPE= 'text' readonly='1' style='background-color:#CCCCCC; text-align: right; font-weight:bold;' NAME='TOT_".$element['ID_ZONE']."_ALL_MES_LIGNE_0' ".$element['ATTRIB_OBJET']."></TD>\n";
															}	
												...
												if (count($affiche_totaux_mat_Frml) > 1) {
													if(count($mat_codes_ligne)){
														...
														foreach($mat_liste_mes as $i_mes => $nom_champ_mes){
															...
															if(count($mat_codes_colonne)){
																...
																foreach($mat_codes_colonne as $col => $elem_col){
																	//foreach($mat_liste_mes as $i_mes => $nom_champ_mes){
																		if( $affiche_totaux_mat_Frml[$i_mes] == true ){
																			$html.="\t\t<TD  align='center' bgcolor='#CCCCCC'>"."<INPUT TYPE= 'text' readonly='1' style='background-color:#CCCCCC; text-align: right; font-weight:bold;' NAME='TOT_".$element['ID_ZONE']."_".$mat_liste_mes[$i_mes]."_COLONNE_0_".$elem_col['code']."' ".$element['ATTRIB_OBJET']."></TD>\n";
																			$html	.= "<script language='javascript' type='text/javascript'>"."\n";
																...
																//foreach($mat_liste_mes as $i_mes => $nom_champ_mes){
																	if( $affiche_totaux_mat_Frml[$i_mes] == true ){
																		$html	.="\t\t<TD  align='center' bgcolor='#CCCCCC'>";
																		if(!in_array($mat_liste_mes[$i_mes],$tab_champ_cache))
																			$html	.="<INPUT TYPE= 'text' readonly='1' style='background-color:#CCCCCC; text-align: right; font-weight:bold;' NAME='TOT_".$element['ID_ZONE']."_".$mat_liste_mes[$i_mes]."_COLONNE_0"."' ".$element['ATTRIB_OBJET'].">";
																		$html	.="</TD>\n";
																	}else{
																		$html.="\t\t<TD  align='center' bgcolor='#CCCCCC'>";
																		if(!in_array($mat_liste_mes[$i_mes],$tab_champ_cache))
															...
															if(count($mat_codes_colonne)){
																...
																if($k==1)
																	$html		.="\t\t<td $rowspan_chp_tot align='center' bgcolor='#CCCCCC'  style='vertical-align:middle'>$br_chp_tot"."<INPUT TYPE= 'text' readonly='1' style='background-color:#CCCCCC; text-align: right; font-weight:bold;' NAME='TOT_".$element['ID_ZONE']."_ALL_MES_0' ".$element['ATTRIB_OBJET']."></TD>\n";
															}
															$k++;
															$html 		    .="\n\t</TR>\n";
														}
														...
														if(count($mat_codes_colonne)){
															...
															foreach($mat_codes_colonne as $col => $elem_col){
																$html		.="\t\t<TD align='center' bgcolor='#CCCCCC'>"."<INPUT TYPE= 'text' readonly='1' style='background-color:#CCCCCC; text-align: right; font-weight:bold' NAME='TOT_".$element['ID_ZONE']."_ALL_MES_COLONNE_0_".$elem_col['code']."' ".$element['ATTRIB_OBJET']."></TD>\n";
															}
															$html		.="\t\t<td $colspan_chp_tot align='center' bgcolor='#CCCCCC' style='vertical-align:middle'>$br_chp_tot"."<INPUT TYPE= 'text' readonly='1' style='background-color:#CCCCCC; text-align: right; font-weight:bold;' NAME='TOT_".$element['ID_ZONE']."_ALL_MES_0' ".$element['ATTRIB_OBJET']."></TD>\n";
														}
														$html 		    .="\n\t</TR>\n";
														...
														$html		.="\t\t<td align='center' bgcolor='#CCCCCC'  valign='middle'>"."<INPUT TYPE= 'text' readonly='1' style='background-color:#CCCCCC; text-align: right; font-weight:bold' NAME='TOT_".$element['ID_ZONE']."_ALL_MES_0' ".$element['ATTRIB_OBJET']."></TD>\n";
													...
													foreach($mat_liste_mes as $i_mes => $nom_champ_mes){
														...
														if(count($mat_codes_colonne)){
															...
															$html			.="\t\t<TD colspan='".count($affiche_totaux_mat_Frml)."' align='center' bgcolor='#CCCCCC'>"."<INPUT TYPE= 'text' readonly='1' style='background-color:#CCCCCC; text-align: right; font-weight:bold;' NAME='TOT_".$element['ID_ZONE']."_ALL_MES_0' ".$element['ATTRIB_OBJET']."></TD>\n";
														}else{
															$html		.="\t\t<TD colspan='".count($affiche_totaux_mat_Frml)."' align='center' bgcolor='#CCCCCC'>"."<INPUT TYPE= 'text' readonly='1' style='background-color:#CCCCCC; text-align: right; font-weight:bold;' NAME='TOT_".$element['ID_ZONE']."_ALL_MES_0"."' ".$element['ATTRIB_OBJET']."></TD>\n";
														}
														$html 		    .="\n\t</TR>\n";
							...
							elseif( ($element['TYPE_OBJET']=='liste_radio' or $element['TYPE_OBJET']=='liste_checkbox' or $element['TYPE_OBJET']=='text_valeur_multiple') && !(isset($element['OBJET_MATRICIEL']) && ($element['OBJET_MATRICIEL'] == true)) ){
									...
									$nb_tr 				= intval($nb_choix / 2) + 1;
									$reste_div 		= $nb_choix % 2;
									...
									//else 
									$html.= "\t<TR class='$class_ligne'>\n";
									$html.= "\t\t<TD $rowspan_tr valign=top class='police_gras'>&nbsp;";
									$html.= $this->recherche_libelle_zone($element['ID_ZONE'],$langue);
									$html.= "&nbsp;</TD>\n";
									for( $tr=0; $tr<$rowspan; $tr++ ){
											...
											for($j=0; $j<2; $j++){
													...
													if(isset($liste_choix[$ordre])){
															...
															$label = $liste_choix[$ordre]['LIBELLE'];
															if($element['TYPE_OBJET']=='liste_radio'){
															...
															if($element['TYPE_OBJET']=='liste_radio'){
																$html.="<LABEL for='".$element['CHAMP_PERE']."_0_".$liste_choix[$ordre][$this->get_champ_extract($element['CHAMP_PERE'])]."'>".$label."</LABEL><INPUT NAME='$name' TYPE='".$type_objet."' ".$element['ATTRIB_OBJET']." ";
															} else {
																$html.= "&nbsp;".$label."&nbsp;<INPUT NAME='$name' TYPE='".$type_objet."' ".$element['ATTRIB_OBJET']." ";
															}
															...
															if (strlen($element['CHAMP_PERE'])>30){
																if ($this->conn->databaseType == 'mssqlnative' || $this->conn->databaseType == 'mssql'){
																	$taille_max_car =   30;                                                               
																		...
																		$taille_max_extract=strlen($element['CHAMP_PERE']);
																}else{
																	$taille_max_car =   31;
																}
															}else{
...
//----------------------------------------------------------------------
		...
		*/
		...
		function generer_frame_grille($id_theme, $langue ,$id_systeme){
			...
			//$html 			.= "<style type='text/css' media='screen'>@import '../css/formulaire_senegal.css';</style>"."\n";
			$html 			.= "<div align=center>"."\n";
			$html 			.= "<FORM NAME='form1' ACTION='questionnaire.php?theme_frame=".$id_theme.$id_systeme."' METHOD='POST' class='grille_ligne'>"."\n";
			$html 			.= "<TABLE class='table-questionnaire' border='1'>"."\n\t";
			...
			$html 			.= "\t\t<TD ROWSPAN='".$nb_niv_1."' class='".$classe_fond."'></TD>\n";
			$html 			.= "\t\t<TD ROWSPAN='".$nb_niv_1."' class='".$classe_fond."'>#</TD>\n";
			...
			for ($j = 0; $j < $NB_LIGNE_ECRAN; $j++){ // Session 46 : NB_LIGNES_FRAME iterations
				...
				if(isset($GLOBALS['PARAM']['TEACHERS_MANAGEMENT']) && $GLOBALS['PARAM']['TEACHERS_MANAGEMENT'] && in_array(''.$id_theme.$id_systeme,$GLOBALS['PARAM']['TEACHERS_LIST_THEMES'])){
					...
					if (array_key_exists($position, $GLOBALS['PARAM']['TEACHER_DETAILS_THEMES']) && $GLOBALS['PARAM']['TEACHER_DETAILS_THEMES'][$position]<>''){
						...
						//$html .= "<a name='".$j."'></a>";
						$html .= "\t\t<TD class='".$classe_fond."' id='".$classe_fond.'_'.$j.'_'.$i_TD."'></TD>";
					}
				}
				//Fin Ajout HEBIE
			...
			foreach($dico as $element){
				   ...
				   // Ajout d'une condition de test Alassane
					if ( trim($element['TABLE_FILLE'])<>'' and trim($element['CHAMP_FILS'])<>'' ) {//A revoir pour mobile
						$result_nomenc 	= $this->requete_nomenclature($element['TABLE_FILLE'], $element['CHAMP_FILS'], $element['CHAMP_PERE'], $langue ,$id_systeme, $element['SQL_REQ']);
					}
					...
					if( (isset($element['OBJET_MATRICIEL']) && ($element['OBJET_MATRICIEL'] == true)) ){
							...
							foreach($mat_codes_colonne[$grp_mat] as $col => $elem_col){
								...
								if(!$element['AFFICH_VERTIC_MES']){
									foreach($mat_liste_mes[$grp_mat] as $i_mes => $nom_champ_mes){					
									  // Pour chaque colonne, on imprime la (ou les) zone(s) de texte
										$html		.= "\t\t<TD nowrap='nowrap' align='center' valign='middle' CLASS='".$classe_fond."' id='".$classe_fond.'_'.$j.'_'.$i_TD."'>";
									...
									$html		.= "\t\t<TD nowrap='nowrap' align='center' style='vertical-align:middle;' COLSPAN=". $nb_mesures[$grp_mat] ." CLASS='".$classe_fond."' id='".$classe_fond.'_'.$j.'_'.$i_TD."'>";
									$cpt_mes = 0;
						...
						switch ($element['TYPE_OBJET']){
							...
							case 'liste_checkbox':
								...
								break;
							case 'text_valeur_multiple':{//A revoir pour mobile
								$zone_ref = get_zone_ref_of($element['ID_ZONE_REF']);
								$result_nomenc 		= $this->requete_nomenclature($zone_ref['TABLE_FILLE'],$zone_ref['CHAMP_FILS'], $zone_ref['CHAMP_FILS'], $langue ,$id_systeme, $zone_ref['SQL_REQ']);
								foreach($result_nomenc as $element_result_nomenc){
									$html       .= "\t\t<TD ".$style_align_middle." class='".$classe_fond."' id='".'ligne-paire'.'_'.$j.'_'.$i_TD."'><div class='input_text'></div><input name_base='".$element['CHAMP_PERE']."_#_".$element_result_nomenc[$this->get_champ_extract($zone_ref['CHAMP_FILS'])]."' id='".$element['CHAMP_PERE']."_".$j."_".$element_result_nomenc[$this->get_champ_extract($zone_ref['CHAMP_FILS'])]."' name='".$element['CHAMP_PERE']."_".$j."_".$element_result_nomenc[$this->get_champ_extract($zone_ref['CHAMP_FILS'])]."' type='text' value=''>";
									$html       .= "</TD>\n";
									$i_TD++;
									( ($aff_total_vertic == true)  && ($j == 0) ) ? ( $last_tr_tot .= '<td></td>' ) : ( $last_tr_tot .= '' ) ;
								}
							...
							case 'combo':
							case 'systeme_combo':
								$html		.= "\t\t<TD ".$style_align_middle." class='".$classe_fond."' id='".'ligne-paire'.'_'.$j.'_'.$i_TD."'>\n";
								$html		.= "<SELECT ".$element['ATTRIB_OBJET']." name_base='".$element['CHAMP_PERE']."_#".'\' id=\''. $element['CHAMP_PERE']."_".$j .'\' '."name=".$element['CHAMP_PERE']."_".$j." data-mini='true'>\n";
						...
						case 'text':
						case 'systeme_text':
							$html 	        .= "\t\t<TD ".$style_align_middle." class='".$classe_fond."'  id='".'ligne-paire'.'_'.$j.'_'.$i_TD."'><div class='input_text'></div><input name_base='".$element['CHAMP_PERE']."_#' id='".$element['CHAMP_PERE']."_".$j."' name='".$element['CHAMP_PERE']."_".$j."' type='text' value=''>";
							$html 	        .= "</TD>\n";
							$i_TD++;
							( ($aff_total_vertic == true)  && ($j == 0) ) ? ( $last_tr_tot .= '<td></td>' ) : ( $last_tr_tot .= '' ) ;
							break;
						...
						case 'hidden_field':
							$html 	        .= "\t\t<INPUT ".' name_base=\''.$element['CHAMP_PERE']."_#".'\' ID=\''. $element['CHAMP_PERE']."_".$j .'\' '."NAME=".$element['CHAMP_PERE']."_".$j." TYPE='hidden'";
			...
			//A revoir le colspan
			$html .= "\t<TR>\n";
			$html .= "\t\t\t<td colspan='20' align='left'><a href='#' class='ui-btn ui-icon-plus ui-btn-icon-notext ui-corner-all ui-btn-inline' id='b_add'>&nbsp;</a></td>\n";
			$html .= "\t</TR>\n";
	...
	$requete				="		SELECT	DICO_THEME.*, DICO_THEME_SYSTEME.*, DICO_ZONE.*, DICO_ZONE_SYSTEME.*
		...
		if(!$affiche_vertic_mes	){
			...
			$tabindex = array();
				...
				$html 			.= "\t"."<TR>"."\n";
		$html 			.= "\t\t<TD COLSPAN='$nb_colspan_1' class='ligne-impaire'></TD>";
		for( $ligne=0; $ligne < $NB_LIGNE_ECRAN; $ligne++ ){
			$tabindex[$ligne] = ($ligne * 1000) +1 ;
			$html .= "\t\t<TD COLSPAN=".($nb_colspan_2 * $nb_cols_zone_mat)." class='ligne-impaire'>"."<INPUT tabindex='".$tabindex[$ligne]."' NAME=DELETE"."_".$ligne." TYPE='checkbox' VALUE=1></TD>";
			$html .="\t\t<TD class='col_sep' ROWSPAN='100' style='width: 1px; padding: 1px;'></TD>\n";
		}
					...
					if( (isset($element['OBJET_MATRICIEL']) && ($element['OBJET_MATRICIEL'] == true)) &&  ( $element['TYPE_OBJET'] <> 'dimension_ligne')  && ( $element['TYPE_OBJET'] <> 'dimension_colonne') ){
										...
										for( $ligne=0; $ligne < $NB_LIGNE_ECRAN; $ligne++ ){
											...
											foreach($mat_liste_mes[$grp_mat] as $i_mes => $nom_champ_mes){					
											  // Pour chaque colonne, on imprime la (ou les) zone(s) de texte
												...
												$html		.= "\t\t<TD  colspan='".($colspan_cur_zone_mat)."' nowrap='nowrap' align='center' valign='middle' CLASS='".$classe_fond."'>";
						...
						elseif ( $element['TYPE_OBJET'] == 'combo' or $element['TYPE_OBJET'] == 'systeme_combo' ){
									...
									for( $ligne=0; $ligne < $NB_LIGNE_ECRAN; $ligne++ ){
											$tabindex[$ligne]++;
											$html 	.= "\t\t<TD COLSPAN=".($nb_colspan_2 * $nb_cols_zone_mat)." class='".$classe_fond."'>";
											$html	.= "<SELECT ".$element['ATTRIB_OBJET']." tabindex='".$tabindex[$ligne]."' ".' ID=\''. $element['CHAMP_PERE']."_".$ligne .'\' '."NAME='".$element['CHAMP_PERE']."_".$ligne."' data-mini='true'>\n";
											$html 	.= "\t\t\t<OPTION VALUE=''>---</OPTION>\n"; 
						...
						if( (isset($dico[$i_elem]['OBJET_MATRICIEL']) && ($dico[$i_elem]['OBJET_MATRICIEL'] == true)) &&  ( $dico[$i_elem]['TYPE_OBJET'] <> 'dimension_ligne')  && ( $dico[$i_elem]['TYPE_OBJET'] <> 'dimension_colonne') ){
							...
							elseif ( $dico[$i_elem]['TYPE_OBJET'] == 'combo' or $dico[$i_elem]['TYPE_OBJET'] == 'systeme_combo' ){
										...
										for( $ligne=0; $ligne < $NB_LIGNE_ECRAN; $ligne++ ){
												$tabindex[$ligne]++;
												$html 	.= "\t\t<TD COLSPAN=".($nb_colspan_2 * $nb_cols_zone_mat)." class='".$classe_fond."'>";
												$html		.= "<SELECT ".$dico[$i_elem]['ATTRIB_OBJET']." tabindex='".$tabindex[$ligne]."' ".' ID=\''. $dico[$i_elem]['CHAMP_PERE']."_".$ligne .'\' '."NAME='".$dico[$i_elem]['CHAMP_PERE']."_".$ligne."' data-mini='true'>\n";
												$html 	.= "\t\t\t<OPTION VALUE=''>---</OPTION>\n"; 
        ...
        $nb_colspan_2 = 2;
            ...
            // S'il s'agit d'une liste_radio, on a besoin de deux lignes (1 hor. et 1 vert.)
  			...
  			$key_tab_groupe 				= array_keys($tab_groupe);
				...
				$html 			.= "\t"."<TR>"."\n";
        $html 			.= "\t\t<TD COLSPAN='$nb_colspan_1' class='".$classe_fond."'></TD>";
        for( $ligne=0; $ligne < $NB_LIGNE_ECRAN; $ligne++ ){
                        $tabindex[$ligne] = ($ligne * 1000) + 1;
						$html .= "\t\t<TD COLSPAN='$nb_colspan_2' class='".$classe_fond."'>"."<INPUT tabindex='".$tabindex[$ligne]."' NAME=DELETE"."_".$ligne." TYPE='checkbox' VALUE=1></TD>";
				}
				$html 			.= "\t".'</TR>'."\n";
        $ii = 0;
				foreach($dico as $element){
        ...
        $nb_colspan_2 = 2;
            ...
            // S'il s'agit d'une liste_radio, on a besoin de deux lignes (1 hor. et 1 vert.)
  			...
  			$key_tab_groupe 				= array_keys($tab_groupe);
				...
				// Affichage de la colonne contenant l'ic�ne ?????????Ade s�lection des �l�ments � supprimer
				$html 			.= "\t"."<TR>"."\n";
        $html 			.= "\t\t<TD COLSPAN='$nb_colspan_1' class='td_center ligne-titre'></TD>";
        for( $ligne=0; $ligne < $NB_LIGNE_ECRAN; $ligne++ ){
                        $tabindex[$ligne] = ($ligne * 1000) + 1;
						$html .= "\t\t<TD COLSPAN='$nb_colspan_2' class='td_center'>"."<INPUT tabindex='".$tabindex[$ligne]."' NAME=DELETE"."_".$ligne." TYPE='checkbox' VALUE=1></TD>";
				}
				$html 			.= "\t".'</TR>'."\n";
        $ii = 0;
				$ordre_eff =	-1;
   ...
   function generer_frame_mat_grille($id_theme, $langue ,$id_systeme){
        ...
        $nb_colspan 		= 0;
				...
				$affiche_totaux_champ_text = array();
  				$html 			.= "\t"."<TR>"."\n";
        		$html 			.= "\t\t<TD ROWSPAN='$nb_rowspan_1' class='".$classe_fond."'></TD>";				
				foreach($dico as $element){
  			...
  			$html 			.= "\t"."<TR  class='ligne-impaire'>"."\n";
				foreach($dico as $element){
						if ( trim($element['TYPE_OBJET']) == 'systeme_liste_radio' 
                            ...
                            foreach($result_nomenc as $element_result_nomenc){		
                                ...
                                $html 		.="\t\t"."<TD ROWSPAN='$nb_rowspan_2' nowrap  align='center'>".$element_result_nomenc['LIBELLE']."</TD>\n";
                                // Fin Modification alassane
                            }
						}
						elseif ( trim($element['TYPE_OBJET']) == 'systeme_valeur_multiple' ){
				...
				for ($ligne = 0; $ligne < $NB_LIGNE_ECRAN; $ligne++){
					...
					// TODO: remonter dans la css
						...
						// Pour chaque Ligne
						$i_TD = 0;
						$html .= "\t\t<TD class='".$classe_fond."'  id='".$classe_fond.'_'.$ligne.'_'.$i_TD."'><INPUT NAME=DELETE"."_".$ligne." TYPE='checkbox' VALUE=1></TD>";
						$i_TD++;
						foreach($dico as $element){
								//$i_TD++;
								if( ($element['TYPE_OBJET'] == 'systeme_valeur_unique') and ($element['BOUTON_INTERFACE']=='popup') ){
											$html 	.= "\t\t<TD nowrap class='".$classe_fond."' id='".$classe_fond.'_'.$ligne.'_'.$i_TD."'>";
											if( trim($element['CHAMP_INTERFACE'])<>trim($element['CHAMP_PERE']) ){
													$html .= "<INPUT ID='".$element['CHAMP_PERE']."_$ligne' name='".$element['CHAMP_PERE']."_$ligne' type='hidden' value='$".$element['CHAMP_PERE']."_$ligne' >"; 
								...
								elseif($element['TYPE_OBJET'] == 'combo' or $element['TYPE_OBJET'] == 'systeme_combo'){
										$result_nomenc 		= $this->requete_nomenclature($element['TABLE_FILLE'],$element['CHAMP_FILS'], $element['CHAMP_PERE'], $langue ,$id_systeme, $element['SQL_REQ']);
										$html 	.= "\t\t<TD class='".$classe_fond."' id='".$classe_fond.'_'.$ligne.'_'.$i_TD."'>";
										$html		.= "<SELECT ".$element['ATTRIB_OBJET']." ".' ID=\''. $element['CHAMP_PERE']."_".$ligne .'\' '."NAME='".$element['CHAMP_PERE']."_".$ligne."' data-mini='true'>\n";
										$html 	.= "\t\t\t<OPTION VALUE=''>---</OPTION>\n"; 
										foreach($result_nomenc as $element_result_nomenc){
												//Un valeur du combo pour chaque valeur de la nomenclature 
												$html 	.= "\t\t\t<OPTION VALUE=".$element['CHAMP_PERE']."_".$ligne."_".$element_result_nomenc[$this->get_champ_extract($element['CHAMP_PERE'])].">"; 
								...
								elseif($element['TYPE_OBJET'] == 'systeme_valeur_multiple'){
										...
										foreach($result_nomenc as $element_result_nomenc){		
												foreach($tab_champ_val_multi as $champ){
													if(isset($champ['TYPE_OBJET']) && $champ['TYPE_OBJET']=='hidden_field'){
														$html .= "\t\t<TD class='".$classe_fond."' nowrap id='".$classe_fond.'_'.$ligne.'_'.$i_TD."'>";
								...
								foreach ($this->id_themes as $id_theme){
										...
										switch ($type_frame){
												...
												case "Matrice" :
													...
													break;
												default :
													print 'Attention verifier le Dico <BR>';
													break;
										}
								}
						}
