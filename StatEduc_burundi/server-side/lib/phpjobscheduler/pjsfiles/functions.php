...
function show_logs($qstart)
{
 ...
 if(count($rows))
 {
  ...
  $next_link="<strong><a href=\"".$GLOBALS['SISED_URL']."administration.php?val=feedbacks_auto_error&start=$next_logs\">Show Next $num logs &gt;&gt;</a>
             </strong><br><br><br>";
  ...
  {      
    ...
    $curl->success(function($instance) {
  	...
  	$curl->complete(function($instance) {
  		echo 'call completed <br/>';  //echo "<pre>"; print_r($instance);
  	}); 
     //echo "<br/>URL : ".$url."<br/>";   
    $curl->get($url);	              
  }
...
function getTaskScheduleState_db()
{
 $dbc = dbc::instance();
 $result = $dbc->prepare("SELECT state FROM task_schedule;");
 $rows = $dbc->executeGetRows($result);
 if(count($rows))
 {
  return $rows[0]["state"];
...
function getTaskSchedulePeriod_db() 
{
   $dbc = dbc::instance();
   $result = $dbc->prepare("SELECT period FROM task_schedule;");
   $rows = $dbc->executeGetRows($result);
   if(count($rows))
   {
...
function updateTaskSchedule_db($state, $period)
{ 
 $dbc = dbc::instance();
 $result = $dbc->prepare("UPDATE task_schedule SET state=".$state.", period='".$period."'");
 $result = $dbc->execute($result);
}
