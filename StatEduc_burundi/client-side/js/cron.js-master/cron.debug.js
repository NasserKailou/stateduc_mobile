...
goog.string.compareVersions = function(a, b) {
  for (var c = 0, d = goog.string.trim(String(a)).split("."), e = goog.string.trim(String(b)).split("."), f = Math.max(d.length, e.length), g = 0;0 == c && g < f;g++) {
    var h = d[g] || "", k = e[g] || "", p = RegExp("(\\d*)(\\D*)", "g"), t = RegExp("(\\d*)(\\D*)", "g");
    do {
      var q = p.exec(h) || ["", "", ""], r = t.exec(k) || ["", "", ""];
      if (0 == q[0].length && 0 == r[0].length) {
        break;
      }
      var c = 0 == q[1].length ? 0 : parseInt(q[1], 10), s = 0 == r[1].length ? 0 : parseInt(r[1], 10), c = goog.string.compareElements_(c, s) || goog.string.compareElements_(0 == q[2].length, 0 == r[2].length) || goog.string.compareElements_(q[2], r[2]);
    } while (0 == c);
  }
  return c;
};
...
var sage = {cron:{}};
sage.cron.Job = function(a, b) {
  this.cronspec_ = sage.cron.Job.toSpec_(a);
  this.block_ = b;
  this.last_at = null;
  this.calcNextAt_();
};
sage.cron.Job.toSpec_ = function(a) {
  if (goog.isString(a)) {
    return new sage.cron.Spec(a);
  }
  goog.asserts.assert(a instanceof sage.cron.Spec);
  return a;
};
sage.cron.Job.prototype.calcNextAt_ = function() {
  var a = new Date, b;
  if (this.last_at) {
    for (b = this.cronspec_.next(this.last_at);b < a;) {
      b = this.cronspec_.next(b);
    }
  } else {
    b = this.cronspec_.next(a);
  }
  this.next_at = b;
};
sage.cron.Job.prototype.getNextTimeout = function(a) {
  a || (a = new Date);
  this.next_at || this.calcNextAt_();
  return Math.max(10, this.next_at - a);
};
...
sage.cron.Scheduler = function() {
  this.jobs_ = [];
  this.running_ = !1;
  this.stored_timeout_ = void 0;
};
sage.cron.Scheduler.prototype.add = function(a) {
  this.jobs_.push(a);
  this.running_ && this.next();
};
sage.cron.Scheduler.prototype.tolerance_ = 1E3;
sage.cron.Scheduler.prototype.check = function() {
  this.running_ && this.next_scheduled_at_ < new Date - this.tolerance_ && this.restart();
};
sage.cron.Scheduler.prototype.remove = function(a) {
  for (var b;-1 !== (b = this.jobs_.indexOf(a));) {
    this.jobs_.splice(b);
  }
};
sage.cron.Scheduler.prototype.start = function() {
  this.running_ || (this.running_ = !0, this.next(), this.startCheck_());
};
sage.cron.Scheduler.prototype.startCheck_ = function() {
  this.stored_interval_ || (this.stored_interval_ = setInterval(this.check.bind(this), 6E4));
};
sage.cron.Scheduler.prototype.stopCheck_ = function() {
  this.stored_interval_ && clearInterval(this.stored_interval_);
};
...
sage.cron.Scheduler.prototype.stop = function() {
  this.running_ && (this.running_ = !1, this.clearTimeout_(), this.stopCheck_());
};
sage.cron.Scheduler.prototype.next = function() {
  var a = !this.running_, a = (a = a || Boolean(this.stored_timeout_)) || !Boolean(this.jobs_.length);
  a || (a = this.getNextJob(), this.executeJob_(a));
};
sage.cron.Scheduler.prototype.executeJob_ = function(a) {
  var b = new Date, c = a.getNextTimeout(null);
  this.next_scheduled_at_ = new Date(b.getTime() + c);
...
sage.cron.Scheduler.prototype.setTimeout_ = function(a, b) {
  var c = setTimeout(goog.bind(a, this), b);
  this.setTimeoutValue_(c);
};
sage.cron.Scheduler.prototype.clearTimeout_ = function() {
  var a = this.getTimeoutValue_();
  clearTimeout(a);
  this.clearTimeoutValue_();
};
sage.cron.Scheduler.prototype.setTimeoutValue_ = function(a) {
  this.stored_timeout_ = a;
};
...
sage.cron.Scheduler.prototype.getNextJob = function() {
  for (var a = this.jobs_[0], b, c = 1, d = this.jobs_.length;c < d;c++) {
    b = this.jobs_[c], b.next_at < a.next_at && (a = b);
  }
  return a;
};
sage.util = {};
sage.util.array = {};
sage.util.array.uniq = function(a) {
  for (var b = a.length, c, d, e = 0;e < b;e += 1) {
    for (c = a[e], d = e + 1;-1 !== (d = a.indexOf(c, d));) {
      a.splice(d, 1), b--;
    }
  }
  return a;
...
sage.util.date = {};
sage.util.date.addTime = function(a, b) {
  var c = a.getTime();
  a.setTime(c + b);
  return a;
};
sage.util.date.addSeconds = function(a, b) {
  var c = a.getSeconds();
  a.setSeconds(c + b);
  return a;
};
sage.util.date.addMinutes = function(a, b) {
  var c = a.getMinutes();
  a.setMinutes(c + b);
  return a;
};
sage.util.date.addHours = function(a, b) {
  var c = a.getHours();
  a.setHours(c + b);
  return a;
};
sage.util.date.addDays = function(a, b) {
  var c = a.getDate();
  a.setDate(c + b);
  return a;
};
sage.util.date.addMonths = function(a, b) {
  var c = a.getMonth();
  a.setMonth(c + b);
  return a;
};
sage.util.date.addYears = function(a, b) {
  var c = a.getFullYear();
  a.setFullYear(c + b);
  return a;
};
sage.cron.syntax = {};
sage.cron.syntax.AbstractParser = function(a, b) {
  this.parser = a;
  this.regex = b;
};
sage.cron.syntax.AbstractParser.prototype.test = function(a) {
  this.regex.lastIndex = 0;
  return this.regex.test(a);
};
sage.cron.syntax.SingleParser = function(a) {
  sage.cron.syntax.AbstractParser.call(this, a, RegExp("^" + a.allow + "$"));
};
goog.inherits(sage.cron.syntax.SingleParser, sage.cron.syntax.AbstractParser);
sage.cron.syntax.SingleParser.prototype.parse = function(a) {
  var b = this.parser.range;
  a = parseInt(a, 10) - b.from;
  return[b.valueAt(a)];
};
sage.cron.syntax.RangeParser = function(a) {
  sage.cron.syntax.AbstractParser.call(this, a, RegExp("^" + a.allow + "-" + a.allow + "$"));
};
goog.inherits(sage.cron.syntax.RangeParser, sage.cron.syntax.AbstractParser);
sage.cron.syntax.RangeParser.prototype.parse = function(a) {
  var b = a.split("-");
  a = parseInt(b[0], 10) - this.parser.range.from;
  b = parseInt(b[1], 10) - this.parser.range.from;
  return this.parser.range.getValues(null, null).slice(a, b + 1);
};
sage.cron.syntax.CommaParser = function(a) {
  sage.cron.syntax.AbstractParser.call(this, a, /\,/);
};
goog.inherits(sage.cron.syntax.CommaParser, sage.cron.syntax.AbstractParser);
sage.cron.syntax.CommaParser.prototype.parse = function(a) {
  a = a.split(",");
  for (var b = a.length, c = [], d = 0;d < b;d++) {
    c.push.apply(c, this.parser.parse(a[d]));
  }
  return c;
};
sage.util.Range = function(a, b) {
  if (a > b) {
    throw Error("sage.util.Range: from is larger than to");
  }
  this.from = a;
  this.to = b;
  this.length = 1 + b - a;
};
sage.util.Range.prototype.valueAt = function(a) {
  return 0 > a || a >= this.length ? void 0 : this.from + a;
};
sage.util.Range.prototype.indexOf = function(a) {
  var b = -1;
  a >= this.from && a <= this.to && (b = a - this.from);
  return b;
};
sage.util.Range.prototype.getValues = function(a, b) {
  "number" !== typeof a && (a = this.from);
  "number" !== typeof b && (b = this.to);
  return this.getValuesInternal(a, b);
};
sage.util.Range.prototype.getValuesInternal = function(a, b) {
  var c, d;
  if (a > b || a < this.from || b > this.to) {
    throw Error("sage.util.Range: values are out of range");
  }
  c = 1 + b - a;
  d = Array(c);
  if (0 < c) {
...
sage.util.RangeParser = function(a, b) {
  this.range = new sage.util.Range(a, b);
  this.parsers = [];
};
sage.util.RangeParser.prototype.parse = function(a) {
  for (var b = this.parsers.length, c = [], d, e = 0;e < b;e++) {
    if (d = this.parsers[e], d.test(a)) {
      c = d.parse(a);
      sage.util.array.uniq(c);
      c.sort(function(a, b) {
        return a - b;
      });
      break;
...
sage.cron.syntax.AllParser = function(a) {
  sage.cron.syntax.AbstractParser.call(this, a, /^\*$/);
};
goog.inherits(sage.cron.syntax.AllParser, sage.cron.syntax.AbstractParser);
sage.cron.syntax.AllParser.prototype.parse = function(a) {
...
goog.math.longestCommonSubsequence = function(a, b, c, d) {
  ...
  for (var p = [], h = e, k = f;0 < h && 0 < k;) {
    c(a[h - 1], b[k - 1]) ? (p.unshift(d(h - 1, k - 1)), h--, k--) : g[h - 1][k] > g[h][k - 1] ? h-- : k--;
  }
  return p;
};
...
sage.cron.syntax.IncrementParser = function(a) {
  sage.cron.syntax.AbstractParser.call(this, a, RegExp("^[^/]+/" + a.allow + "$"));
};
goog.inherits(sage.cron.syntax.IncrementParser, sage.cron.syntax.AbstractParser);
sage.cron.syntax.IncrementParser.prototype.parse = function(a) {
  var b = a.split("/");
  a = this.parser.parse(b[0]);
  for (var b = parseInt(b[1], 10), c = [], d = 0, e = a.length;d < e;d += b) {
    c.push(a[d]);
  }
  return c;
};
sage.cron.SpecParser = function(a, b, c) {
  sage.util.RangeParser.call(this, a, b);
  this.allow = c;
...
sage.cron.AliasSpecParser = function(a, b, c, d) {
  sage.cron.SpecParser.call(this, a, b, c);
  this.aliasMap = new goog.structs.Map(d);
  a = this.aliasMap.getKeys();
  this.aliasRegEx = RegExp("(" + a.join("|") + ")", "g");
};
goog.inherits(sage.cron.AliasSpecParser, sage.cron.SpecParser);
sage.cron.AliasSpecParser.prototype.resolveAliases = function(a) {
  var b = this.aliasMap;
  return a.replace(this.aliasRegEx, function(a) {
    return b.get(a, a);
  });
};
sage.cron.AliasSpecParser.prototype.parse = function(a) {
  return sage.cron.AliasSpecParser.superClass_.parse.call(this, this.resolveAliases(a));
};
sage.cron.SpecSecondParser = new sage.cron.SpecParser(0, 59, "[1-5]?[0-9]");
sage.cron.SpecMinuteParser = new sage.cron.SpecParser(0, 59, "[1-5]?[0-9]");
sage.cron.SpecHourParser = new sage.cron.SpecParser(0, 23, "(([0-1]?[0-9])|([2][0-3]))");
sage.cron.SpecDayParser = new sage.cron.SpecParser(1, 31, "(([0]?[1-9])|([1-2][0-9])|([3][0-1]))");
sage.cron.SpecMonthParser = new sage.cron.AliasSpecParser(1, 12, "(([0]?[1-9])|([1][0-2]))", {jan:1, feb:2, mar:3, apr:4, may:5, jun:6, jul:7, aug:8, sep:9, oct:10, nov:11, dec:12});
sage.cron.SpecWeekdayParser = new sage.cron.AliasSpecParser(1, 7, "([0]?[1-7])", {sun:1, mon:2, tue:3, wed:4, thu:5, fri:6, sat:7});
sage.cron.Spec = function(a) {
  a = a.trim();
  a in sage.cron.Spec.aliases_ && (a = sage.cron.Spec.aliases_[a]);
  this.spec = a;
  var b = a.split(" ");
  if (6 !== b.length) {
    throw a = "CronSpec only accepts specifications with 6 parts:" + a + " is invalid", Error(a);
  }
  this.seconds = sage.cron.SpecSecondParser.parse(b[0]);
  this.minutes = sage.cron.SpecMinuteParser.parse(b[1]);
  this.hours = sage.cron.SpecHourParser.parse(b[2]);
  this.days = sage.cron.SpecDayParser.parse(b[3]);
  this.months = sage.cron.SpecMonthParser.parse(b[4]);
  this.weekdays = sage.cron.SpecWeekdayParser.parse(b[5]);
};
sage.cron.Spec.prototype.next = function(a) {
  var b = new Date;
  b.setTime(a.getTime());
  sage.util.date.addSeconds(b, 1);
  b.setMilliseconds(0);
  for (var c = (a = this.seconds.length) && 60 !== a, d = this.minutes.length, e = d && 60 !== d, f = this.hours.length, g = f && 24 !== f, h = this.weekdays.length, k = h && 7 !== h, p = this.days.length, t = p && 31 !== p, q = this.months.length, r = q && 12 !== q, s = !1, m, n;!s;) {
    if (m = b.getSeconds(), c && -1 === this.seconds.indexOf(m)) {
      n = !0;
      for (var l = 0;l < a;l++) {
        if (this.seconds[l] >= m) {
          b.setSeconds(this.seconds[l]);
          n = !1;
          break;
        }
      }
      n && (b.setSeconds(0), sage.util.date.addMinutes(b, 1));
    } else {
      if (m = b.getMinutes(), e && -1 == this.minutes.indexOf(m)) {
        n = !0;
        for (l = 0;l < d;l++) {
          if (this.minutes[l] >= m) {
            b.setMinutes(this.minutes[l]);
            n = !1;
            break;
          }
        }
        n && (b.setMinutes(0), sage.util.date.addHours(b, 1));
      } else {
        m = b.getHours();
        if (g && -1 == this.hours.indexOf(m)) {
          n = !0;
          for (l = 0;l < f;l++) {
            if (this.hours[l] >= m) {
              b.setHours(this.hours[l]);
              n = !1;
              break;
            }
          }
          ...
          b.setMinutes(0);
          if (n) {
            sage.util.date.addDays(b, 1);
            b.setHours(0);
            continue;
          }
        }
        m = b.getDay() + 1;
        if (k && -1 == this.weekdays.indexOf(m)) {
          n = !0;
          for (l = 0;l < h;l++) {
            if (this.weekdays[l] >= m) {
              sage.util.date.addDays(b, this.weekdays[l] - m - 1);
              n = !1;
              break;
            }
          }
          if (n) {
            sage.util.date.addDays(b, 8 - m);
            b.setSeconds(0);
            b.setMinutes(0);
            b.setHours(0);
            continue;
        ...
        m = b.getDate();
        if (t && -1 == this.days.indexOf(m)) {
          n = !0;
          for (l = 0;l < p;l++) {
            if (this.days[l] >= m) {
              for (;b.getDate() != this.days[l];) {
                b.setDate(this.days[l]);
              }
              n = !1;
              break;
            }
          ...
          b.setHours(0);
          b.setMinutes(0);
          n && (b.setDate(1), sage.util.date.addMonths(b, 1));
        } else {
          if (m = b.getMonth() + 1, r && -1 == this.months.indexOf(m)) {
            n = !0;
            for (l = 0;l < q;l++) {
              if (this.months[l] >= m) {
                b.setMonth(this.months[l] - 1);
                n = !1;
                break;
              }
            }
            ...
            b.setHours(0);
            b.setDate(1);
            n && (b.setMonth(0), sage.util.date.addYears(b, 1));
          } else {
            s = !0;
          }
        }
...
sage.cron.Spec.aliases_ = {"@second":"* * * * * *", "@minute":"0 * * * * *", "@halfminute":"0,30 * * * * *", "@hour":"0 0 * * * *", "@halfhour":"0 0,30 * * *", "@daily":"0 0 0 * * *", "@monthly":"0 0 0 1 * *", "@yearly":"0 0 0 1 1 *", "@weekly":"0 0 0 * * 1", "@weekday":"0 0 0 * * 2-6"};
sage.cron.Spec.next = function(a, b) {
  return b.next(a);
};
sage.cron.Spec.parse = function(a) {
  return new sage.cron.Spec(a);
};
var cron = {js:{}};
