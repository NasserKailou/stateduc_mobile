...
function Arborescence(Atop,Aleft,img_dir) {
	...
	this.heightItem = 22;
	this.top = Atop?Atop:15;
	this.left = Aleft?Aleft:3;
	...
	this.img_dir = img_dir?img_dir:"client-side/image/treeview/";
...
function arbo_buildItem(objItem) {
	...
	if (whithChild) {
		htmlCode = htmlCode + linkContainerStart + '&nbsp;<A HREF="javascript:clickOnItem(' + this.id + ',' + objItem.id + ');" CLASS="' + this.nodeClass + '" target="_self" '+ 'title="'+ objItem.libelle +'">' + objItem.libelle + '</A>&nbsp;' + linkContainerEnd;
	} else {
		htmlCode = htmlCode + linkContainerStart + '&nbsp;<A HREF="javascript:clickOnItem(' + this.id + ',' + objItem.id + ');" CLASS="' + this.itemClass + '" target="_self"' + 'title="'+ objItem.libelle +'">' + objItem.libelle + '</A>&nbsp;' + linkContainerEnd;
	}
	if (this.browser.ie4) {
		...
		if (ItemId < objArbo.all.length) {
			...
			if (objItem.link != null) {
				...
				if (objItem.target=="popup") {
					...
					//alert(objItem.target +" = "+objItem.link);
				} else {
