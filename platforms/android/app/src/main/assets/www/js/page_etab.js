...
var stmPageEtab = {
	...
	addNextTr : function(input, lastTr, inc) {
		if ($(lastTr).find('input[name='+input+']').length == 0 && $(lastTr).find('select[name='+input+']').length == 0) {
			var lastNum = ($(lastTr).find('.inc_number').html() * 1) - 1;
			var nextNum = lastNum + inc;
			var nextTr = this.createNextTr(lastTr, nextNum);
			if ($(nextTr).find('input[name='+input+']').length == 0 && $(nextTr).find('select[name='+input+']').length == 0) {
				inc++;
	...
	initPageData : function(idQst, idPage) {
		...
		$.each(datas, function(index, value) {
			...
			if (isGrille) {
				var lastTr = curr.findLastTr();
				var key = value.getKey();
				if (value.getType() == 'radio') {
					key = 	key.substr(0, key.indexOf("#"));
				}
				if ($('.grille_ligne input[name='+ value.getKey() +']').length == 0 && $('.grille_ligne select[name='+ value.getKey() +']').length == 0) { 
